...
@section('content')
    <div class="card card-info">
        <div class="card-header">
            <h3 class="card-title float-left">إضافة سعر المقعد</h3>
        </div>
        <!-- /.card-header -->
        <form class="form-horizontal" method="POST" action="{{ route('admin.seat-prices.store') }}">
            @csrf
            <div class="card-body">
                <!-- حقل نوع التلبيسة -->
                    ...
                    <div class="col-sm-10">
                        <select name="category_id" id="category_id" class="select2 form-control @error('category_id') is-invalid @enderror" required>
                            ...
                            @foreach($seatCovers as $seatCover)
                                <option value="{{ $seatCover->id }}">{{ $seatCover->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <select name="seat_count_id" id="seat_count_id" class="select2 form-control @error('seat_count_id') is-invalid @enderror" required>
                            <option value="" selected disabled>اختر عدد المقاعد</option>
                            @foreach($seatCounts as $seatCount)
                                <option value="{{ $seatCount->id }}" {{ old('seat_count_id') == $seatCount->id ? 'selected' : '' }}>{{ $seatCount->name }}</option>
                            @endforeach
                        </select>
                ...
                <div class="form-group row">
                    ...
                    <div class="col-sm-10">
                        <input type="number" name="price" id="price" class="form-control @error('price') is-invalid @enderror" min="0" required>
