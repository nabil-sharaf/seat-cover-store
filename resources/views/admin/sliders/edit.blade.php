...
@section('content')
    <div class="container mt-5">
        <h2 class="mb-4">تعديل الـ slider</h2>
        ...
        <form action="{{ route('admin.sliders.update',$slider->id) }}" method="POST" enctype="multipart/form-data" class="row g-4">
            ...
            <div class="col-md-12">
                ...
                <textarea id='slider-text' name="description" class="form-control @error('text') is-invalid @enderror" rows="4" placeholder="أدخل نص السلايدر">{{ old('description', $slider->description ?? '') }}</textarea>
                @error('description')
                <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
